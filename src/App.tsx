...
export default function App() {
  ...
  const translateTechnicalProfile = async (fromLang: 'en' | 'pt', toLang: 'en' | 'pt') => {
    setTranslatingIdx('tech-profile');
    setTranslationError(null);
    try {
      const sourceText = configForm[fromLang].additionalSkillsText;
      if (!sourceText || !sourceText.trim()) return;

      const translated = await translateApiCall(sourceText, toLang);

      setConfigForm(prev => ({
        ...prev,
        [toLang]: {
          ...prev[toLang],
          additionalSkillsText: translated
        }
      }));
    } catch (e: any) {
      console.error("Failed to translate technical profile", e);
      setTranslationError(e.message || "Failed to translate Technical Profile. Check if your Gemini key is configured correctly.");
    } finally {
      setTranslatingIdx(null);
    }
  };
                        ...
                        <div className="flex items-center justify-between border-b border-white/5 pb-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              const fromL = editLang;
                              const toL = editLang === 'en' ? 'pt' : 'en';
                              translateTechnicalProfile(fromL, toL);
                            }}
                            className="flex items-center gap-1 text-[10px] font-mono uppercase bg-orange-500/10 text-orange-500 border border-orange-500/20 px-2.5 py-1 rounded-md hover:bg-orange-500/20 transition-all font-bold disabled:opacity-50"
                          >
                            {translatingIdx === 'tech-profile' ? (
                                <>
                                  <span className="w-2.5 h-2.5 border-2 border-orange-500 border-t-transparent rounded-full animate-spin"></span>
                                  Translating...
                                </>
